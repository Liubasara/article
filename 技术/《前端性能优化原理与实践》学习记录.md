...
- **不带 async 和 defer 的 JS** 会阻塞 DOM 的解析（浏览器为了不做无用功解析多余的 DOM），且同时会触发渲染（一些 JS 文件里用到了 DOM 的样式操作所以需要渲染获取样式），而触发渲染时若 CSS 尚未全部加载完成，就会一不小心连带渲染一起阻塞，所以需要将 JS 文件的加载尽量放在文档的后面。
