...
Web 组件指的是一套用于增强 DEOM 行为的工具，包括影子 DOM、自定义元素和 HTML 模板。要注意的是：

- 并没有统一的 Web Components 规范：每个 Web 组件都在一个不同的规范中定义
- 有些 Web 组件如影子 DOM 和自定义元素，已经出现了向后不兼容的版本问题（实名吐槽 chrome 浏览器还有自家的 polymer 框架哈哈哈）
- 浏览器实现极其不一致

> 由于存在这些问题，因此使用 Web 组件通常需要引入一个 Web 组件库，比如 Polymer

#### 20.11.1 HTML 模板

`<template>`标签中的内容不会被渲染到页面中，通过 template 元素的 content 属性可以取得其内在的`DocumentFragment`的引用。

#### 20.11.2 影子 DOM

影子 DOM（shadow DOM）可以将一个完整的 DOM 树作为节点添加到父 DOM 树中，这样可以实现 DOM 封装，意味着 CSS 样式和 CSS 选择符可以限制在影子 DOM 子树而不是整个顶级 DOM 树中。

可以通过`attachShadow()`方法创建并添加给有效的 HTML 元素。该方法需要一个`shadowRootInit`对象，返回影子 DOM 的实例。该对象必须包含一个 mode 属性，值为 open 或 closed。对 open 的影子 DOM 可以用 HTML 元素上获得，而 close 的引用则无法像这样获取。

> 一般来说，需要创建保密（closed）影子 DOM 的场景很少。虽然这可以限制通过影子宿主访问影 子 DOM，但恶意代码有很多方法绕过这个限制，恢复对影子 DOM 的访问。简言之，不能为了安全而 创建保密影子 DOM。
>
