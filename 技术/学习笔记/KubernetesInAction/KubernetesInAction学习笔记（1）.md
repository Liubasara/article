...
为了在 Kubernetes 中运行应用，首先需要将应用打包进一个或多个镜像容器，再将那些镜像推送到镜像仓库，然后将**应用的描述**发布到 Kubernetes API 服务器。

这个描述会包括以下几点：

- 容器镜像
- 这些组件如何相互关联
- 哪些组件需要同时运行在同一个节点上，哪些组件不需要同时运行
- 哪些组件为内部或外部提供服务，且通过 IP 地址端口映射，并使其他组件可以发现

（PS：个人理解，有点像 docker-compose.yml 文件）
...
- 简化应用程序部署，开发人员可以自己部署应用程序而不需要了解集群服务器，甚至无需关心自己的程序运行在哪个服务器上（PS：个人理解 K8S 就像把一台台独立的电脑重新在他的调度下变成了一整个资源）

  当然在某些情况下，你也可以指定应用程序在哪些具有资源的机器上运行，比如说对于某些频繁访问磁盘的应用程序，就可以让 Kuberrnetes 只在具有 SSD 的节点中进行选择并运行。具体的使用将在第 3 章描述
...
- 加速持续部署，持续交付，方便回滚，对于开发人员来说可以直接使用线上的环境进行开发，更效率，更安全
