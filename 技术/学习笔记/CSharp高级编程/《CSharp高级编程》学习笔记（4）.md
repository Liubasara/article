...
> public、protected 和 private 是逻辑访问修饰符，而 internal 是一个物理访问修饰符，其边界是一个程序集。
>
> 
>
> 程序集定义——chatGPT：
>
> Q：C# 中不同的 namespace 之间算是不同的程序集吗？
>
> A：在C#中，不同的命名空间（namespace）并不意味着不同的程序集（assembly）。命名空间是一种组织和管理代码的方式，而程序集则是一个独立的可执行文件或库文件。
>
> 在同一个项目中，可以有多个命名空间，但它们都属于同一个程序集。这意味着它们可以相互访问和使用。
>
> 然而，如果你在不同的项目中定义了不同的命名空间，那么它们将会是不同的程序集。每个项目都会生成一个独立的程序集文件（.dll或.exe文件），并且它们之间是相互独立的。在这种情况下，如果你想在一个项目中使用另一个项目中的类型或成员，你需要引用对应的程序集。
>
> 因此，命名空间和程序集是两个不同的概念，它们之间没有直接的关系。命名空间用于组织和管理代码，而程序集用于封装和部署代码。

最外部的类型（也就是 namespace 中的类型）只能定义为 public、internal。不能定义为 private、protected、private protected 或 protected internal。但是嵌套类型（即在类型里面定义的类型）也可被视为一个类的成员，因此也能使用这些修饰符。
...
```csharp
public interface IDissposable
{
  void Dispose();
}

public class MyClass: IDissposable
{
  public void Dispose()
  {
    //
...
上面的代码说明，声明接口在语法上与声明抽象类完全相同。但接口不允许提供接口任何成员的实现方式，所以也不需要`abstreact`关键字。

一般情况下，接口只能包含方法、属性、索引器和事件的声明。接口定义中还不允许声明成员的修饰符，如果需要，就应该由实现的类来声明。

接口名称通常以字母 I 开头，以便知道这是一个接口。

接口的强大之处在于，它可以指向任何实现了该接口的类，例如，可以构造接口数组，其中数组的每个元素都是不同的类（但这些类都实现了这个接口）。
...
`is`和`as`运算符都用于类型转换和判断，但它们不会直接进行强制类型转换，而是会让编译器检查参数是否有实现响应的接口：
...
> 扩展阅读：cast 强制类型转换运算符——chatGPT
>
> 在C#中，cast运算符用于将一个对象转换为另一个类型。它的语法如下：
>
> ```csharp
> targetType result = (targetType)expression;
> ```
>
> 其中，`targetType`是要转换的目标类型，`expression`是要转换的表达式或对象。
>
> 下面是一些示例：
>
> ```csharp
> int num = 10;
> double result = (double)num; // 将int类型转换为double类型
> 
> object obj = "Hello";
> string str = (string)obj; // 将object类型转换为string类型
> ```
>
> 需要注意的是，如果转换不是有效的，将会抛出`InvalidCastException`异常。因此，在进行转换之前，最好使用`is`运算符或`as`运算符进行类型检查。
>
> ```csharp
> if (obj is string)
> {
...
> ```
>
> 或者使用`as`运算符进行安全转换，如果转换失败，将返回`null`。
>
> ```csharp
...
> ```
>
> 总之，cas 运算符可以将一个对象转换为指定的类型，但需要注意进行类型检查以避免异常。
