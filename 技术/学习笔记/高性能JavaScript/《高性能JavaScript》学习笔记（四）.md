...
本章的目的是给你一些关于有效地组织并部署基于 JavaScript 的 Web 应用程序的建议。

### 预处理 JavaScript 文件

预处理器的任务是将输入数据处理成另一种编程语言所使用的数据。预处理 JavaScript 源文件并不会加速你的程序，但它允许你在代码中加入其它语言才有的一些特性，例如用条件插入一些测试代码，来衡量你应用程序的性能。

(PS：比如说大名鼎鼎的 TypeScript，本质上其实就是预处理器)

### JavaScript 紧凑

JavaScript 紧凑指的是剔除代码文件中一切与运行无关的内容，包括注释和不必要的空格，使得下载速度更快，尺寸更小。

### JavaScript 压缩

浏览器请求一个资源时，会发送一个 Accept-Encoding 的HTTP头，让网页服务器知道传输所支持的编码类型，从而改善用户体验。Accept-Encoding 的取值范围是：
...
如果服务器在请求报文中看到这些信息头，它将会选择适当的编码方法，并通过 Content-Encoding 的 HTTP 头来通知浏览器。

gzip 是目前最流行的压缩方式，通常可以将有效载荷减少到 70% 。但要注意的是，除了网页外的一些文件类型，如图片和PDF这种已经经过压缩的资源，就没有必要再进行压缩了。

### 缓存 JavaScript 文件

通过`<html>`标签的 manifest 属性可以指定一个配置文件，该配置文件中列出网站应当被缓存的资源。

**关于缓存问题**

缓存固然可以提高用户体验，但是要如何确保当应用程序更新以后，用户能看到静态内容的最新版本呢？答案是添加版本号，大多情况下，开发者可以向文件名中添加一个版本号或者时间戳，从而确保在更新时浏览器会去下载最新的资源。