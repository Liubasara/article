...
19. 指令：recursive_error_pages 多级错误跳转指令
    ...
    说明：当使用 error_pages 设定多层内部访问时，仍可处理上一层级返回的响应状态码。下面的配置中，如果 recursive_error_pages 为 off，则 Nginx 只会处理一次 error_page 404。当指令值为 on，且 upstream 返回状态码为 502 时，才会调用 upfallback 的内部访问。
    ...
    http {
      ...
      location @fallback {
        ...
        proxy_pass http://backend;
        # 当上游服务器返回 502 状态码时，执行内部请求 @upfallback
        error_page 502 = @upfallback;
      }
      location @upfallback {
        # 当前的所有请求代理到上游服务器 newbackend
        proxy_pass http://newbackend;
      }
    }
    ```

20. 指令：server_tokens 响应服务版本号指令
    ...
    说明：默认在错误信息响应头中增加 Server 以标识 Nginx 的版本号
...
21. 指令：msie_padding mise 响应注视指令
    ...
    说明：在响应状态大于或等于 400 时，会在响应报文中添加注释，使得响应报文大小达到 512 B。仅仅适用于 msie 客户端。该指令配置也支持 chrome 客户端。
